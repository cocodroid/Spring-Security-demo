...
解决：
	 ...
	 *7.basic认证
		basic认证是另一个常用的认证方式，与表单认证不同的是，basic认证
		常用于无状态客户端的验证，比如HttpInvoker或者Web Service的认证，
		这种场景的特点是客户端每次访问应用时，都在请求头部携带认证信
		息，一般就是用户名和密码，因为basic认证会传递明文，所以最好使
		用https传输数据。	
			1). 配置basic验证		
					<http> 
					<http-basic />
		...
		*8标签库	
		     为了在视图层直接访问用户信息，再者就是为了对显示的内容进行权限管理		
			1)配置taglib	
